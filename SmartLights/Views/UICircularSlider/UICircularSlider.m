...
- (void)setup {
    ...
    _tipImg = [UIImage imageNamed:@"brightness_tip_s"];
}
...
- (CGFloat)sliderRadius {
	CGFloat radius = MIN(self.bounds.size.width/2, self.bounds.size.height/2) - _inset;
	radius -= MAX(kLineWidth, kThumbRadius);	
	return radius;
}
- (void)drawThumbAtPoint:(CGPoint)sliderButtonCenterPoint inContext:(CGContextRef)context {
	UIGraphicsPushContext(context);
//    CGFloat x = sliderButtonCenterPoint.x - _tipImg.size.width / 2 - 2;
//    CGFloat y = sliderButtonCenterPoint.y - _tipImg.size.height + 5;
    CGPoint tipPoint = [self tipImagePoint];
    [_tipImg drawAtPoint:tipPoint];
    UIColor* textColor = self.minimumTrackTintColor;
    [textColor setFill];
    NSString* percentage = [NSString stringWithFormat:@"%.0f%%", _value];
    
    NSInteger indent = 0;
    if (_value >= 0 && _value < 9) {
        indent = 8;
    } else if(_value > 9 && _value < 100){
        indent = 4;
    }
//    [percentage drawAtPoint:CGPointMake(x + 12 + indent, y + 4) withAttributes:nil];
    [percentage drawAtPoint:CGPointMake(tipPoint.x + 12 + indent, tipPoint.y + 4) withFont:[UIFont systemFontOfSize:12]];
	UIGraphicsPopContext();
}

-(CGPoint)tipImagePoint{
    CGFloat x = self.thumbCenterPoint.x - _tipImg.size.width / 2 - 2;
    CGFloat y = self.thumbCenterPoint.y - _tipImg.size.height + 5;
...
#pragma mark - Thumb management methods
- (BOOL)isPointInThumb:(CGPoint)point {
    CGPoint thumbPoint = [self tipImagePoint];
	CGRect thumbTouchRect = CGRectMake(thumbPoint.x, thumbPoint.y, _tipImg.size.width, _tipImg.size.height);
...
- (void)panGestureHappened:(UIPanGestureRecognizer *)panGestureRecognizer {
	...
	switch (panGestureRecognizer.state) {
		case UIGestureRecognizerStateChanged: {
			...
			CGPoint sliderStartPoint = CGPointMake(0, 0);
            ...
            if (angle > 1.5* M_PI && angle < 1.75 * M_PI) {
                angle = 1.5 * M_PI;
            }
            if (angle > 1.75 * M_PI) {
                angle = 0;
...
//    return [self isPointInThumb:[gestureRecognizer locationInView:self]];
//}

-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    BOOL receive = [self isPointInThumb:[touch locationInView:self]];
    return receive;
}
